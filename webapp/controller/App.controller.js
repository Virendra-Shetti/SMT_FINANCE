sap.ui.define([
	...
	"use strict";
	jQuery.sap.includeScript("https://cdnjs.cloudflare.com/ajax/libs/jspdf/1.3.5/jspdf.debug.js");
	jQuery.sap.includeScript("https://unpkg.com/jspdf@latest/dist/jspdf.min.js");
...
for(var j=0; j<oModel1.length; j++){
	if(reviedData.empId == oModel1[j].EmpId){
			var objectPage = this.getView().byId("idObjectPLay");
		objectPage.bindElement("EmpSalary>/EmployeeDetail/"+ j);
		...
		onPressSubmit: function () {
		   ...
		   for(var i=0; i<oModel1.length; i++ ){
		   	if(EmpployeeId == oModel1[i].EmpId && months == oModel1[i].Month ){
		   eflag=false;
		   		}
		   		
		   		else if(i == database-1){
		   			 	oModel1.push(oEnrty);
		...
		onGetValue: function () {
			debugger;
			var getMonthArr = this.filteredMonthArray;
			var dropYearValue = this.getView().byId("idComboYear").getValue();
			var oModel1 = this.getView().getModel().getData().EmpSalary;
			...
			if (dropMonthValue) {
				...
				var oFilterTeamName1 = new sap.ui.model.Filter("Year", sap.ui.model.FilterOperator.Contains, dropYearValue);
				var oFilter = new sap.ui.model.Filter({
					filters: [oFilterTeamName, oFilterTeamName1],
					and: true
				});
				var aFilter = oFilter;
				var oList = this.getView().byId("myEmpTbl");
		...
		onPressDownload: function () {
			...
			var tDeduc = Number([payAmtDeduc]) + Number([200]);
			...
			doc.text(76, 180, finString);
			// doc.text(6,190,'In Words');
			doc.setFontSize(8);
			doc.setFontType('bolditalic');
			doc.text(28, 196,
				'"This is system generated payslip does not require signature and the salary will be credited to respective account within 48 hours"'
			);
